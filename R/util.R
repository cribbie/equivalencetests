...
simRepDat <- function() {
    ...
    sim_dependent_samples <- function(sample_size, 
        number_of_sample, sigma_matrix, 
        eps, power) {
        if (power == TRUE) {
            population_means <- rep(0, 
                number_of_sample)
        }
        if (power == FALSE) {
            boundary_diff <- eps
            # population_means <- c(0,
            # boundary_diff, rep(0,
            # number_of_sample-2))
            population_means <- c(0, 
                boundary_diff, rep(boundary_diff/2, 
                  number_of_sample - 
                    2))
        }
        sample_data <- rmvnorm(sample_size, 
            mean = population_means, 
            sigma = sigma_matrix)
        
        return(sample_data)
    }
    make_sigma_matrix <- function(corMatrix, 
        standard_deviation_of_each_sample, 
        number_of_sample) {
        sigma_matrix <- cor2cov(corMatrix, 
            sd = rep(standard_deviation_of_each_sample, 
                number_of_sample))
        return(sigma_matrix)
    }
    k = 3
    ...
    V <- sigma * rho^H
    p <- nrow(V)
    V[cbind(1:p, 1:p)] <- V[cbind(1:p, 
        1:p)] * sigma  #cov matrix 
...
getContrast <- function(k, type) {
    ...
    if (type == "allPW") {
        M <- matrix(0, nrow = k, ncol = k * 
            (k - 1)/2)
        comb <- combn(k, 2)
        M[cbind(comb[1, ], 1:(k * (k - 
            1)/2))] <- -1
        M[cbind(comb[2, ], 1:(k * (k - 
            1)/2))] <- 1
        M <- t(M)
    }
    return(M)
...
#'
#' pairwise_meanDiffs()
pairwise_meanDiffs <- function(sample_means, 
    allcontrasts) {
    if (!require(plyr)) {
        install.packages(plyr)
        library(plyr)
    }
    mean_diffs <- t(allcontrasts %*% 
        sample_means)  #this is the vector of mean differences, ie. mean difference @ k=1, mean of k=2, etc. Matches mean D, pg 245, table 8.5 
...
pairwise_sd <- function(allcontrasts, 
    sigma) {
    var_covar <- allcontrasts %*% sigma %*% 
        t(allcontrasts)  #var-covar matrix of intraindiv differences    
