...
#' eq.corr(dat, .2)
#' }
eq.corr <- function(dat, ei, alpha = 0.05, 
    na.rm = TRUE, ...) {
    dat <- na.omit(dat)
    plot(dat)
    var1 <- x <- dat[, 1]
    ...
    n <- length(var1)
    nresamples <- 10000
    #### Running a traditional t test to
    #### determine if the correlation is
    #### significant######
    t <- corxy/(sqrt((1 - corxy^2)/(n - 
        2)))
    pvalue_tradt <- 1 - pt(abs(t), n - 
        2)
    ...
    #### Running an original two t-test
    #### procedure for equivalence #######
    equivt1 <- (corxy - ei)/sqrt((1 - 
        corxy^2)/(n - 2))
    pvalue1_equivt <- pt(equivt1, n - 
        2)
    equivt2 <- (corxy + ei)/sqrt((1 - 
        corxy^2)/(n - 2))
    pvalue2_equivt <- 1 - pt(equivt2, 
        n - 2)
    ifelse(pvalue1_equivt <= alpha & 
        pvalue2_equivt <= alpha, decis_equivt <- "The null hypothesis that the correlation between var1 and var2 falls outside of the equivalence interval can be rejected.", 
    ...
    ##### Run a two t-test procedure for
    ##### equivlance with Fisher's z
    ##### transformation ####
    zei <- log((1 + ei)/(1 - ei))/2
    zcorxy <- log((1 + corxy)/(1 - corxy))/2
    equivt1_fz <- (zcorxy - zei)/(1/sqrt(n - 
        3))
    pvalue1_fz <- pnorm(equivt1_fz)
    equivt2_fz <- (zcorxy + zei)/(1/sqrt(n - 
        3))
    pvalue2_fz <- 1 - pnorm(equivt2_fz)
    ifelse(pvalue1_fz <= alpha & pvalue2_fz <= 
        alpha, decis_fz <- "The null hypothesis that the correlation between var1 and var2 falls outside of the equivalence interval can be rejected.", 
    ...
    #### Run the resampling version of the
    #### two t-test procedure for
    #### equivalence #####
    resamp <- function(x, m = 10000, 
        theta, conf.level = 0.95, ...) {
        n <- length(x)
        Data <- matrix(sample(x, size = n * 
            m, replace = T), nrow = m)
        ...
        S <- sd(thetastar)
        alpha <- 1 - conf.level
        CI <- quantile(thetastar, c(alpha/2, 
            1 - alpha/2))
    ...
    results <- resamp(x = 1:n, m = nresamples, 
        theta = theta, mat = mat)
    q1 <- quantile(results$ThetaStar, 
        0.05)
    q2 <- quantile(results$ThetaStar, 
        0.95)
    q1negei <- q1 - ei
    q2negei <- q2 - ei
    q1posei <- q1 + ei
    ...
    ifelse(q2negei < 0 & q1posei > 0, 
        decis_rs <- "The null hypothesis that the correlation between var1 and var2 falls outside of the equivalence interval can be rejected.", 
    ...
    stats_tradt <- c(corxy, t, n - 2, 
        pvalue_tradt)
    names(stats_tradt) <- c("Pearson r", 
        "t-statistic", "df", "p-value")
    stats_equivt <- c(corxy, ei, equivt1, 
        pvalue1_equivt, equivt2, pvalue2_equivt, 
        n - 2)
    names(stats_equivt) <- c("Pearson r", 
        "Equivalence Interval", "t-stat 1", 
        "pval_t1", "t-stat 2", "pval_t2", 
        "df")
    stats_fz <- c(corxy, ei, equivt1_fz, 
        pvalue1_fz, equivt2_fz, pvalue2_fz)
    names(stats_fz) <- c("Pearson r", 
        "Equivalence Interval", "z-stat 1", 
        "pval_z1", "z-stat 2", "pval_z2")
    stats_rs <- c(corxy, ei, nresamples, 
        q1, q2)
    names(stats_rs) <- c("Pearson r", 
        "Equivalence Interval", "# of Resamples", 
        "5th Percentile", "95th Percentile")
    out <- list(list(title1, stats_tradt, 
        decis_tradt), list(title2, stats_equivt, 
        decis_equivt), list(title3, stats_fz, 
        decis_fz), list(title4, stats_rs, 
        decis_rs))
    class(out) <- "eq.corr"
    out
}
