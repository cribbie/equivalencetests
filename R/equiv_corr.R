...
equiv_corr <- function(dat, equivint, alpha = 0.05, na.rm = TRUE, 
    ...
    n <- length(var1)
    nresamples <- 10000
    #### Running a traditional t test to determine if the correlation
    #### is significant######
    ...
    names(stats_equivt) <- c("Pearson r", "Equivalence Interval", 
        "t-stat 1", "pval_t1", "t-stat 2", "pval_t2", "df")
    stats_fz <- c(corxy, equivint, equivt1_fz, pvalue1_fz, equivt2_fz, 
        pvalue2_fz)
