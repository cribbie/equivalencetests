...
equiv_t <- function(x, y, equivint, tr = 0.2, alpha = 0.05, varequal = FALSE, ...) {
    ...
    equivt <- function(x, y, equivint, alpha = 0.05, varequal = FALSE, na.rm = TRUE, 
        ...
        if (varequal == TRUE) {
            t1 <- (mean(x) - mean(y) - equivint)/sqrt(((((length(x) - 1) * sd(x)^2) + 
                                                            ((length(y) - 1) * sd(y)^2))/(length(x) + length(y) - 2)) * (1/length(x) + 
                                                                                                                             1/length(y)))
            t2 <- (mean(x) - mean(y) + equivint)/sqrt(((((length(x) - 1) * sd(x)^2) + 
                                                            ((length(y) - 1) * sd(y)^2))/(length(x) + length(y) - 2)) * (1/length(x) + 
                                                                                                                             1/length(y)))
    ...
    if(tr == 0)
        return(equivt(x=x, y=y, equivint=equivint, alpha=alpha, varequal=varequal, 
                      na.rm=FALSE, ...))    
