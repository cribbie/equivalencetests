...
#' 
#' }
eq.1way.ww <- function(dv, group, eps, 
    alpha = 0.05) {
    group <- factor(group)
    size <- table(group)
    ng <- length(size)
    ...
    crit_psisq <- ((ng - 1)/mean(size)) * 
        qf(p = alpha, df1 = ng - 1, df2 = sum(size) - 
    ...
    ret <- data.frame(stat = as.numeric(psisq), 
        df1 = ng - 1, df2 = sum(size) - 
            ng, p.crit = crit_psisq, 
...
print.eq.1way.ww <- function(x, ...) {
    ...
    cat("with degrees of freedom ", x$df1, 
        ",", x$df2, ".\n")
