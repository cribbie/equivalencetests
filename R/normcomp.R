...
normcomp <- function(premean, presd, pren, postmean, postsd, postn, 
    ...
    for (i in 1:length(equivint)) {
        ...
        dft <- (((postsd^2/postn) + (normsd^2/normn))^2)/((postsd^4/(postn^2 * 
            (postn - 1))) + (normsd^4/(normn^2 * (normn - 1))))
    ...
    tk1 <- (postmean - normmean - equivint[2])/sqrt(((((postn - 
        1) * postsd^2) + ((normn - 1) * normsd^2))/(postn + normn - 
        2)) * (1/postn + 1/normn))
    ...
    rownames(results_kendall) <- c("t-test1", "t-test2", "df1", 
        "df2", "pvalue1", "pvalue2")
    out <- list(title1, title2, preresults, decis_pret, title3, 
        results, title4, decism, title5, title6, results_kendall, 
        decisk)
    class(out) <- "normcomp"
    out
}
