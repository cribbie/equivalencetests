...
#' }
normcomp <- function(premean, presd, pren, postmean, 
    postsd, postn, normmean, normsd, normn, alpha = 0.05, 
    ...) {
    equivint <- c(0.5 * (normsd), normsd, 1.5 * 
        (normsd))
    t1 <- c(0, 0, 0)
    t2 <- c(0, 0, 0)
    probt1 <- c(0, 0, 0)
    ...
    for (i in 1:length(equivint)) {
        ...
        ifelse(probt1[i] < alpha & probt2[i] < alpha, 
            decis[i] <- "The treated and normative means are declared equivalent at this Equivalence Interval", 
