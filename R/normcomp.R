...
normcomp <- function(premean, presd, pren, postmean, postsd, postn, normmean, normsd, normn, alpha = 0.05, ...) {
    ...
    for (i in 1:length(equivint)) {
        ...
        dft <- (((postsd^2/postn) + (normsd^2/normn))^2)/((postsd^4/(postn^2 * (postn - 1))) + (normsd^4/(normn^2 * (normn - 
            1))))
    ...
    tk1 <- (postmean - normmean - equivint[2])/sqrt(((((postn - 1) * postsd^2) + ((normn - 1) * normsd^2))/(postn + normn - 
        2)) * (1/postn + 1/normn))
    tk2 <- (postmean - normmean + equivint[2])/sqrt(((((postn - 1) * postsd^2) + ((normn - 1) * normsd^2))/(postn + normn - 
        2)) * (1/postn + 1/normn))
    dfk1 <- postn + normn - 2
    dfk2 <- postn + normn - 2
