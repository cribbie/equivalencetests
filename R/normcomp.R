...
#' }
normcomp <- function(premean, presd, pren, postmean, postsd, postn, normmean, normsd, 
    normn, alpha = 0.05, ...) {
    ...
    preresults <- data.frame(matrix(0, nrow = 1, ncol = 3, dimnames = list(c("Value"), 
        c("t-test", "df", "p-value"))))
    results <- matrix(0, nrow = 6, ncol = 3, dimnames = list(c("t-test1", "t-test2", 
        "df1", "df2", "pvalue1", "pvalue2"), c("EI=.5*sd", "EI=sd", "EI=1.5*sd")))
    ...
    df_pret <- (((presd^2/pren) + (normsd^2/normn))^2)/((presd^4/(pren^2 * (pren - 
        1))) + (normsd^4/(normn^2 * (normn - 1))))
    ...
    for (i in 1:length(equivint)) {
        ...
        dft <- (((postsd^2/postn) + (normsd^2/normn))^2)/((postsd^4/(postn^2 * (postn - 
            1))) + (normsd^4/(normn^2 * (normn - 1))))
    ...
    rownames(results_kendall) <- c("t-test1", "t-test2", "df1", "df2", "pvalue1", 
        "pvalue2")
    out <- list(title1, title2, preresults, decis_pret, title3, results, title4, 
        decism, title5, title6, results_kendall, decisk)
    class(out) <- "normcomp"
    out
}
