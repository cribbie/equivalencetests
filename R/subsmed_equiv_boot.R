...
subsmed_equiv_boot <- function(IV, DV, m, ei, standardize = FALSE, nboot = 10000, 
    ...
    semmod <- structure(c("x->m", "m->y", "m<->m", "y<->y", "x<->x", "bmx", "bym", 
        "erm", "ery", "erx", NA, NA, NA, NA, NA), .Dim = c(5L, 3L), class = "semmod")
    ...
    for (i in 1:nboot) {
        ...
        yb <- newdatb[, 2]
        mb <- newdatb[, 3]
        semmodb <- structure(c("xb->mb", "mb->yb", "mb<->mb", "yb<->yb", "xb<->xb", 
    ...
    ifelse(quantile(bootresults_eq[, 1], alpha) > 0 & quantile(bootresults_eq[, 2], 
        1 - alpha) < 0, decis <- "It is concluded that the mediator explains a substantial proportion of the variability in the relationship between the predictor and outcome", 
