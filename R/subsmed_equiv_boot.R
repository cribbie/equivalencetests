...
#' 
#' This function computes the equivalence and bootstrap-based test of substantial mediation outlined in:
#' Mara, C. & Cribbie, R. A. (submitted). An equivalence-based procedure for assessing substantial mediation.
#' Psychological Methods.
#'
#' This function determines whether a single mediator accounts for a substantial proportion of the relationship
#' between an independent variable (IV) and a dependent variable (DV), where quantifying 'a substantial proportion'
#' depends on the width of the equivalence interval. For a mediator to be said to account for a substantial proportion
#' of the relationship between the IV and DV, the raw relationship between IV and DV should be very similar to the
#' reproduced IV-DV relationship (i.e., IV->M*M->DV). The equivalence interval defines how similar these relationships
...
#' @examples
#' \dontrun{
#' #equivalence interval of .1, a standardized solution, 1000 bootstrap samples, and the default alpha level (.05) 
#' m <- rnorm(500)
#' x <- m + rnorm(500) 
#' y <- m + rnorm(500) 
...
subsmed_equiv_boot <- function(IV, DV, m, ei, standardize = FALSE, nboot = 10000, alpha = 0.05) {
    ...
    semmod <- structure(c("x->m", "m->y", "m<->m", "y<->y", "x<->x", "bmx", "bym", "erm", "ery", "erx", 
        NA, NA, NA, NA, NA), .Dim = c(5L, 3L), class = "semmod")
    ...
    for (i in 1:nboot) {
        ...
        yb <- newdatb[, 2]
        mb <- newdatb[, 3]
        semmodb <- structure(c("xb->mb", "mb->yb", "mb<->mb", "yb<->yb", "xb<->xb", "bmx", "bym", "erm", 
            "ery", "erx", NA, NA, NA, NA, NA), .Dim = c(5L, 3L), class = "semmod")
    ...
    ifelse(quantile(bootresults_eq[, 1], alpha) > 0 & quantile(bootresults_eq[, 2], 1 - alpha) < 0, decis <- "It is concluded that the mediator explains a substantial proportion of the variability in the relationship between the predictor and outcome", 
